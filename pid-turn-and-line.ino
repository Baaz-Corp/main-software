...
long int angleCount1, angleCount2;
...
void PID_control(int set_speed, int rm_direction, int lm_direction)
{
  if (set_flag_100ms == 1)
  {
    ...
    right_speed(motor1_set, rm_direction);   // Changing motor speeds
    left_speed(motor0_set, lm_direction);    // Changing motor speeds
  
    
    if (error <= 1 && error >= -1)  // If very little error
    {
      motor1_diff = set_speed - motor1_set;  // get closer to wanted speed
      motor1_set += motor1_diff/10;   // gradually increasing motor speed
      motor0_set += motor1_diff/10; 
      right_speed(motor1_set, rm_direction);
      left_speed(motor0_set, lm_direction); 
    }
...
void loop()
{
  PID_control(setPoint, right_motor_dir, left_motor_dir);
}
